...
/*
 * Т.к. в уточняющей переписке мне было сказано, что выполнение тестового задания "демонстрирует способность кандидата адекватно
 * понимать бизнес ценность реализуемой функциональности. Обработка ошибок и ограничения не должны противоречить здравому смыслу.
 * Делай дальше на свое усмотрение)" => сделал так как понял :)
 *
 * Для того чтобы приложение собиралось, нужно чтобы на компьютере был установлен conan.
 *
 * Исходя из сигнатур высланных функций, ТОП всегда выдается только для текущей недели. (из условия)
 ...
 * Все остальное по условиям тестового задания.
 *
 * Тесты проводились на базе в 1 млн, количество прогонов каждой функции 100 000 , на ноутбуке core i5 (Ubuntu 16.04). Результаты:
...
int main() {
    ...
    TestWokerMode testWokerMode;
    testWokerMode.run_tests(1000000, 100000); //собрать тестовую базу

    Woker wk;
    for (int  i = 0; i < 12; ++i) {
        wk.user_registered(i,"user"+std::to_string(i));
        wk.user_connected(i);
        wk.user_deal_won(i,time(NULL),rand()%90);
    }

    list<string> lst;
    wk.get_user_top(4,lst);
